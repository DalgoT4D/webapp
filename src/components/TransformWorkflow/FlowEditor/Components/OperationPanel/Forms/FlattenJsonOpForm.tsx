  ...
  const fetchJsonColumns = async (selectedColumn: string) => {
    ...
    try {
      const response = await httpGet(
        session,
        `warehouse/dbt_project/json_columnspec/?source_schema=${action !== 'edit' ? nodeData.schema : inputModels[0].schema
        }&input_name=${action !== 'edit' ? nodeData.input_name : inputModels[0].name
