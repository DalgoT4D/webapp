...
describe('Delete connection', () => {
  ...
  it('delete connection', async () => {
    ...
    const deleteConnSuccess = jest.fn().mockResolvedValueOnce({
      ok: true,
      json: jest.fn().mockResolvedValueOnce({
        success: 1,
      }),
    });
    (global as any).fetch = deleteConnSuccess;

    await act(() => confirmButton.click());

    expect(deleteConnSuccess).toHaveBeenCalled();

    // Mock the delete api call failure
    const deleteConnFailure = jest.fn().mockResolvedValueOnce({
      ok: false,
      json: jest.fn().mockResolvedValueOnce({
        detail: 'something went wrong',
      }),
    });
    (global as any).fetch = deleteConnFailure;

    await act(() => confirmButton.click());

    expect(deleteConnFailure).toHaveBeenCalled();
  });
});
