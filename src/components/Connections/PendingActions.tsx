...
import { httpGet } from '@/helpers/http'; // Import your HTTP helper function
...
const PendingActionsAccordion = () => {
  ...
  >({}); // Mapping of connection_id to connection_name
  const { data: session }: any = useSession();

  useEffect(() => {
    const fetchData = async () => {
      try {
        // Fetch schema change data from the API
        ...
        // Fetch connection data to build the connection name map
      ...
      <AccordionDetails>
        <Box sx={{ width: '100%' }}>
          {schemaChangeData.map((schemaChange: any, index: number) => {
            ...
            // Define the styles for the label based on the schema change type
            const labelStyles = {
