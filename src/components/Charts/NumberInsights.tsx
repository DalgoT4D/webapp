...
export const NumberInsights: React.FC<NumberInsightsProps> = ({ data, type }) => {
              ...
              <Box key={key} sx={{ mr: '50px' }}>
                ...
                >
                  <Box data-testid={`innermostbox-${index}`} sx={{ ml: 1 }}>
                    {data[key] ? (
                      key === 'mode' && data.otherModes && data.otherModes.length > 1 ? (
                        <Tooltip title={`Other modes: ${data.otherModes?.join(', ')}`}>
                          <span>{Math.trunc(data[key] as number).toLocaleString()}</span>
                        </Tooltip>
                      ) : (
