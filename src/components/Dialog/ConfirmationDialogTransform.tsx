...
const ConfirmationDialogTransform: React.FC<ConfirmationDialogProps> = ({ open, handleClose, transformType }) => {
  ...
  return (
    <Dialog open={open} onClose={handleClose} maxWidth='xs'>
      <DialogTitle sx={{ fontSize: '25px', marginBottom: '-10px' }}><strong>Confirmation</strong></DialogTitle>
      <DialogContent>
      ...
      <DialogContent>
        <Typography variant="body2" sx={{ bgcolor: '#00000026', p: 1, borderRadius: 1, fontSize: 16, marginBottom: '8px' }}>
          You have opted to continue using the <strong>{transformType === 'ui' ? 'UI' : 'GitHub'} method</strong> to set up your transformation
        </Typography>
        <Typography variant="body2" color="textSecondary" sx={{ marginBottom: '8px' }}>
          <strong>Note:</strong> Once a method is selected, you cannot change it.
        </Typography>
      </DialogContent>
      <DialogActions sx={{ justifyContent: 'center' }}>
        <Button onClick={handleConfirm} autoFocus sx={{ bgcolor: 'rgba(0,0,0,0.6)', color: 'white', width: '90%', 
          '&:hover': {
            color: 'white',
      ...
      <DialogActions sx={{ justifyContent: 'center' }}>
        <Button onClick={handleClose} variant="outlined" sx={{ color: 'rgba(0,0,0,0.6)', width: '90%' }}>
          Cancel
        </Button>
      </DialogActions>
    </Dialog>
